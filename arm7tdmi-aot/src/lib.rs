...
pub fn compile_rom_with_seeds_full_v2(
    ...
    compile_rom_with_seeds_full_v3(
        rom, rom_base, entry_pc, entry_mode, seeds,
        replay_thumb_fn, step_thumb_fn, abort_thumb_fn,
        cpu_offsets, fetch_only_thumb_fn, replay_arm_fn, bios_bytes,
        load_32_fn, idle_cycle_fn, None,
    )
}

/// Phase-4 v3: also accepts bus.store_32 trampoline for F11 STR (and
/// future F9 STR / F14 PUSH) inline IR. Forwards to v4 with no
/// ldr_word fn (F11 LDR remains in trampoline path).
pub fn compile_rom_with_seeds_full_v3(
    rom: &[u8],
    rom_base: u32,
    entry_pc: u32,
    entry_mode: Mode,
    ...
    compile_rom_with_seeds_full_v4(
        rom, rom_base, entry_pc, entry_mode, seeds,
        replay_thumb_fn, step_thumb_fn, abort_thumb_fn,
        cpu_offsets, fetch_only_thumb_fn, replay_arm_fn, bios_bytes,
        load_32_fn, idle_cycle_fn, store_32_fn, None,
    )
}

/// Phase-4 v4: also accepts ldr_word trampoline for F11 LDR sp-rel
/// inline IR. Handles I14 misaligned-LDR ROR + cpsr.C side effect
/// inside the extern (caller IR doesn't update flags around the call).
pub fn compile_rom_with_seeds_full_v4(
    rom: &[u8],
    rom_base: u32,
    entry_pc: u32,
    entry_mode: Mode,
    ...
    compile_rom_with_seeds_full_v5(
        rom, rom_base, entry_pc, entry_mode, seeds,
        replay_thumb_fn, step_thumb_fn, abort_thumb_fn,
        cpu_offsets, fetch_only_thumb_fn, replay_arm_fn, bios_bytes,
...
/// Phase-4 v5: also accepts ldr_half + store_16 trampolines for F10
/// LDRH/STRH inline IR.
pub fn compile_rom_with_seeds_full_v5(
    rom: &[u8],
    rom_base: u32,
    entry_pc: u32,
    entry_mode: Mode,
    ...
    compile_rom_with_seeds_full_v6(
        rom, rom_base, entry_pc, entry_mode, seeds,
        replay_thumb_fn, step_thumb_fn, abort_thumb_fn,
        cpu_offsets, fetch_only_thumb_fn, replay_arm_fn, bios_bytes,
        load_32_fn, idle_cycle_fn, store_32_fn, ldr_word_fn,
        ldr_half_fn, store_16_fn, None, None,
    )
}

/// Phase-4 v6: also accepts load_8 + store_8 trampolines for F9
/// LDRB/STRB inline IR.
pub fn compile_rom_with_seeds_full_v6(
    rom: &[u8],
    rom_base: u32,
    entry_pc: u32,
    entry_mode: Mode,
...
fn compile_rom_with_seeds_full_v7_with_pr(
    ...
    if let Some(s32) = store_32_fn {
        compiler.register_store_32(s32);
    }
    if let Some(lw) = ldr_word_fn {
        compiler.register_ldr_word(lw);
    }
    if let Some(lh) = ldr_half_fn {
        compiler.register_ldr_half(lh);
    }
    if let Some(s16) = store_16_fn {
        compiler.register_store_16(s16);
    }
    if let Some(l8) = load_8_fn {
        compiler.register_load_8(l8);
    }
